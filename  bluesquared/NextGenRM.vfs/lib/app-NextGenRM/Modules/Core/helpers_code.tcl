...
proc nextgenrm_Code::save {args} {
	...
	switch -- [lindex $args 0] {
		profile {
			...
			set profile($profile(Store),table) [.profile.container.nb.f1.frame1.listbox.listbox get 0 end]
			set profile($profile(Store),table) [lrange $profile($profile(Store),table) 0 end-1] ;# Get rid of the empty list generated by an empty line in the listbox
