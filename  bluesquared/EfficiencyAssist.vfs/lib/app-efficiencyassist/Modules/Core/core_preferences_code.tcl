...
proc eAssistPref::saveConfig {} {
    #****f* saveConfig/eAssistPref
    # AUTHOR
    #	Casey Ackels
    #
    # COPYRIGHT
    ...
    set fd [open [file join $program(Home) profile_imports.txt] w]
    ...
    set fd1 [open [file join $mySettings(Home) mySettings.txt] w]
    
    foreach value [array names mySettings] {
        chan puts $fd1 "mySettings($value) $mySettings($value)"
        #puts $mySettings($value)
        ${log}::debug Writing mySettings : mySettings($value) $mySettings($value)
    }
